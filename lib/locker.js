...
export async function buildLocker(athleteId, lifespan = DEFAULT_LIFESPAN) {
  const shoes = await sql`
    select strava_gear_id, name, distance_m, retired
      from shoes
  ...
  return { locker, milesThisYear: toMiles(totals?.metres) };
}
